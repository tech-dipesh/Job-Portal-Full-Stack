...
export const assignUsersToCompanies=async (req, res)=>{
  ...
  try {
    if(!company_id || !uid){
      return res.status(201).json({message: "Please Enter the company_id And uid"})
    }
    ...
    if(check[0].exists){
      return res.status(201).json({message: "User is Already Assign to the Companies."})
    }
...
export const searchCompany=async(req, res)=>{
  const {q}=req.query;
  if(!q){
    return res.status(201).json({message: "Please Enter Search Query"})
  }
  try {
...
export const searchUsers=async(req, res)=>{
  ...
  try {
    const {rows}=await connect.query("select fname, lname uid from users where fname or lname ilike $1", [`%${q}%`])
    return res.status(200).json({message: rows})  
  } catch (error) {
    return res.status(201).json({message: error.message})    
  }
